...
class EGDSimulator():
    ...
    def computeValue(self, tag):
        if tag.type in ["REAL", "LREAL"]:
            tag.lastvalue = tag.lastvalue if tag.lastvalue != None else (
                -1000.0 + random.random() * 2000.0)
            tag.lastvalue += 0 if self.fixed else -10 + random.random() * 20
        elif tag.type == "DINT":
            tag.lastvalue = tag.lastvalue if tag.lastvalue != None else random.randrange(
                -1000, 1000)
            ...
            tag.lastvalue = tag.lastvalue if tag.lastvalue != None else random.randrange(
                0, 2)
            tag.lastvalue = tag.lastvalue if (
                self.fixed or random.randrange(0, 10000) < 9998) else not tag.lastvalue
    ...
    def produce(self, producer, exchange):
        if self.isRunning():
            ...
            tm = time.localtime()
            exchange.lasttimestamp = "" + str(self.count) + " at " + str(tm.tm_hour).zfill(
                2) + ":" + str(tm.tm_min).zfill(2) + ":" + str(tm.tm_sec).zfill(2)
            hdr = egdmodel.EGDHeader.newHeaderFor(
                producer.producerid, exchange.exchangenumber, self.count).header
            payload = struct.pack(
                exchange.codec, *self.computeValues(exchange))
            self.sock.sendto(
                hdr+payload, (self.address, egdmodel.UDP_DATA_PORT))
            threading.Timer(float(exchange.period) / 1000.0,
                            self.produce, [producer, exchange]).start()
